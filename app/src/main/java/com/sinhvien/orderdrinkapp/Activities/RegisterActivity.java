...
//đảm bảo rằng chuỗi chỉ chứa các ký tự từ (ký tự chữ và số và dấu gạch dưới) và dài từ 1 đến 50 ký tự.
        String checkspaces = "\\A\\w{1,50}\\z";

// "\\A" : So khớp với phần đầu của chuỗi.
// "\\w" : Khớp với một ký tự từ đơn (chữ và số hoặc dấu gạch dưới).
// "{1,50}" : Bộ định lượng chỉ ra rằng mẫu trước đó ( \\w) phải được khớp từ 1 đến 50 lần.
// "\\z" : So khớp với phần cuối của chuỗi.
